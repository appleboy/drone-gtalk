...
func main() {
	...
	app.Flags = []cli.Flag{
		cli.StringFlag{
			...
			Usage:  "gtalk host",
			EnvVar: "PLUGIN_HOST,GOOGLE_HOST,HOST,INPUT_HOST",
		},
		cli.StringFlag{
			Name:   "username",
			Usage:  "gtalk user email",
			EnvVar: "PLUGIN_USERNAME,GOOGLE_USERNAME,USERNAME,INPUT_USERNAME",
		},
		cli.StringFlag{
			Name:   "oauthtoken",
			Usage:  "OAuthToken provides go-xmpp with the required OAuth2 token used to authenticate",
			EnvVar: "PLUGIN_OAUTH_TOKEN,OAUTH_TOKEN,INPUT_OAUTH_TOKEN",
		},
		cli.StringFlag{
			Name:   "oauthscope",
			...
			EnvVar: "PLUGIN_OAUTH_SCOPE,OAUTH_SCOPE,INPUT_OAUTH_SCOPE",
		},
		cli.StringSliceFlag{
			Name:   "to",
			Usage:  "send message to user",
			EnvVar: "PLUGIN_TO,TO,INPUT_TO",
		},
		cli.StringSliceFlag{
			Name:   "message",
			Usage:  "gtalk message",
			EnvVar: "PLUGIN_MESSAGE,MESSAGE,INPUT_MESSAGE",
		},
		cli.BoolFlag{
			Name:   "match.email",
